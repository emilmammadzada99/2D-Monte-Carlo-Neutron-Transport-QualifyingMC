...
#include <numeric> // std::accumulate ve std::inner_product için
...
int get_group(double energy) {
    ...
    int group = 9;
    for (size_t g = 0; g < group_energy_boundaries.size(); ++g) {
...
void qualifying_monte_carlo(int num_neutrons) {
    ...
    // --- Simulation Counters and Tallies ---
    ...
    // Flux tallies for each energy group
    std::vector<long long> fuel_surf_neu_num(10, 0);

    // --- Neutron Initial State Generation ---
    std::vector<double> neutron_energies;
    std::exponential_distribution<double> maxwellian_dist(1.0);
    ...
    for (int i = 0; i < num_neutrons; ++i) {
        double r_rand = dist(rng) * dist(rng);
    ...
    for (int i = 0; i < num_neutrons; ++i) {
        ...
        while (alive) {
            ...
            if (d_interaction < d_boundary) {
                ...
                if (region == 0) {
                    fuel_surf_neu_num[get_group(energy)]++;
                }
    ...
    // --- Data Calculation for Output ---
    std::vector<double> group_energy_boundaries = {3e+1, 3e+0, 3e-1, 3e-2, 3e-3, 3e-4, 3e-5, 3e-6, 3e-7, 3e-8};
    double fuel_area = M_PI * r_fuel * r_fuel;
    double scaling_factor = 10.0 * num_neutrons;

    std::vector<double> flux_y(10);
    for (size_t i = 0; i < 10; ++i) {
        flux_y[i] = static_cast<double>(fuel_surf_neu_num[i]) / (fuel_area * scaling_factor);
    }

    // Trapezoidal rule for normalization
    double flux_sum = 0.0;
    for (size_t i = 0; i < 9; ++i) {
        flux_sum += (flux_y[i] + flux_y[i+1]) / 2.0 * (group_energy_boundaries[i] - group_energy_boundaries[i+1]);
    }

    std::vector<double> flux_y_norm(10);
    for (size_t i = 0; i < 10; ++i) {
        if (flux_sum > 0) {
            flux_y_norm[i] = flux_y[i] / flux_sum;
        } else {
            flux_y_norm[i] = 0.0;
        }
    }

    // --- Watt Spectrum Calculation and Normalization ---
    std::vector<double> watt_energy_points;
    for (double e = 0.01; e <= 30.0; e += (30.0 - 0.01) / 500.0) {
        watt_energy_points.push_back(e);
    }

    std::vector<double> watt_flux;
    for (double e : watt_energy_points) {
        watt_flux.push_back(0.453 * std::sinh(std::sqrt(2.29 * e)) * std::exp(-1.036 * e));
    }

    // Trapezoidal rule for Watt spectrum normalization
    double watt_sum = 0.0;
    for (size_t i = 0; i < watt_flux.size() - 1; ++i) {
        watt_sum += (watt_flux[i] + watt_flux[i+1]) / 2.0 * (watt_energy_points[i+1] - watt_energy_points[i]);
    }

    std::vector<double> watt_flux_norm;
    for (double f : watt_flux) {
        if (watt_sum > 0) {
            watt_flux_norm.push_back(f / watt_sum);
        } else {
            watt_flux_norm.push_back(0.0);
        }
    }

    // --- Writing data to a text file ---
    std::ofstream outputFile("flux_data.txt");
    if (outputFile.is_open()) {
        outputFile << "Energy (MeV)\tSimulated_Flux_Norm\tWatt_Flux_Norm\n";

        // Write simulated flux data
        for (size_t i = 0; i < 10; ++i) {
            outputFile << group_energy_boundaries[i] << "\t" << flux_y_norm[i] << "\n";
        }

        outputFile << "\n"; // Separator

        // Write Watt spectrum data
        for (size_t i = 0; i < watt_energy_points.size(); ++i) {
             outputFile << watt_energy_points[i] << "\t" << watt_flux_norm[i] << "\n";
        }

        outputFile.close();
        std::cout << "Flux data saved to flux_data.txt" << std::endl;
    } else {
...
int main() {
    qualifying_monte_carlo(50); // Example run
    return 0;
}
